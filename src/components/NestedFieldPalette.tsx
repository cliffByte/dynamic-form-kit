...
// Restricted field types that cannot be added to nested forms
const RESTRICTED_NESTED_TYPES: FormField['type'][] = [
  'ui_section',
  'step_section',
  'array',
  'map',
...
// Filter out restricted types for nested forms
const nestedFieldTypes = fieldTypes.filter(
  (field) => !RESTRICTED_NESTED_TYPES.includes(field.type),
);
  ...
  return (
    <div
      ...
      <div className='flex items-start gap-4'>
        <div className='p-2 rounded-md transition-colors duration-200 bg-primary/10 text-primary group-hover:bg-primary/15'>
          {icon}
        </div>
        <div className='flex-1 min-w-0'>
          <div className='font-semibold text-sm mb-0.5 transition-colors text-foreground group-hover:text-primary'>
            {label}
          </div>
...
export function NestedFieldPalette() {
  // Group fields by category (excluding restricted types)
  const fieldsByCategory = nestedFieldTypes.reduce(
    (acc, field) => {
    ...
    {} as Record<keyof typeof fieldCategories, typeof nestedFieldTypes>,
  );
    ...
    <Card className='border-border w-full shadow-sm h-fit'>
      <CardHeader className='pb-3 border-b border-border bg-muted/30'>
        ...
        <p className='text-xs text-muted-foreground mt-1'>
          Drag fields to the canvas to build your nested form
        </p>
      </CardHeader>
      <CardContent className='p-4'>
        <div className='space-y-5'>
          {(
            ...
            // Skip category if no fields in it after filtering
            if (!fieldsByCategory[category]?.length) return null;
